...
경제학의 \index{정보 비대칭}정보 비대칭(Information Asymmetry) 이론은 판매자가 구매자보다 더 많은 정보를 가질 때 시장의 신뢰가 무너지고 거래 비용이 증가함을 설명한다\cite{akerlof1970market}.\sidenote{조지 애커로프(George Akerlof)는 \enquote{레몬 시장(The Market for Lemons)} 논문에서 정보 비대칭이 어떻게 시장 실패를 초래하는지 증명했다. 판매자가 구매자보다 더 많은 정보를 가질 때, 구매자는 품질을 확신할 수 없어 평균적인 가격만 지불하려 하고, 결국 좋은 품질의 제품(Peach)은 시장에서 사라지고 나쁜 제품(Lemon)만 남게 되는 역선택(Adverse Selection)이 발생한다.} 마케팅에서의 투명성은 이 비대칭을 의도적으로 해소하는 행위다. 브랜드가 먼저 약점을 공개하면 고객은 숨겨진 위험을 찾기 위해 방어적인 태도를 취할 필요가 없어진다. 즉, 정보의 완전성은 고객이 \enquote{내가 모르는 함정이 있을지 모른다}는 불안감을 내려놓게 만드는 장치다.
...
% Nodes
\node[block] (complete) at (-2.8, 2) {정보의 완전성\\\footnotesize(판단의 재료)};
\node[block] (context) at (2.8, 2) {맥락의 공유\\\footnotesize(해석의 기준)};
